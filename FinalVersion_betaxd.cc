...
void czytaj(void){
	...
	x=readcond(fd, &c, 1,0,0,50);
	if (x<0) {
		cout << "Blad odczytu! " << endl; 
		exit();
		}
	else if (x==0) {
		cout << "Urzadzenie nie odpowaiada!" << endl;
		exit();
	}
...
void * showtime (void * arg){
	while(1){
		pthread_mutex_lock(&muteks);
		cout << " Dioda swiecila lacznie przez  " << czascalkowity << " sekund " << endl;
		pthread_mutex_unlock(&muteks);
	sleep(5);
	}return(NULL);
...
void ustaw(void){
		...
		if (fd<0) {cout <<<< "Blad! Port nie zostal otwarty do odczytu! " << endl;
